(() => {
  ...
  async function load(){
    try{
      ...
      if(initialNode&&Array.isArray(initial)){render(initial);return;}
      const items=site?.fetchJSON?await site.fetchJSON('/api/store-products',{headers:{Accept:'application/json'}},9000):await fetch('/api/store-products').then(r=>r.json());
      render(items);
    }catch{
